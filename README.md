...
**チャットボット（Atte サポート）**<br />
全画面右下に表示される AI チャットボットです。OpenAI API により、会員一覧・勤務日・勤務時間などについて、データベースの内容に基づいて回答します。本日・昨日・今週・今月の勤務登録者や勤務詳細（勤務開始・勤務終了・休憩時間・勤務時間）の質問に対応しています。利用には `.env` に `OPENAI_API_KEY` の設定が必要です。

### レスポンシブ対応

全画面でスマートフォン（SP）表示に対応しています。768px 以下ではヘッダーに三本線のハンバーガーメニューを表示し、タップでナビゲーションを開閉できます。勤務一覧・日付一覧の時間表示は SP では時・分のみ（秒は非表示）です。
